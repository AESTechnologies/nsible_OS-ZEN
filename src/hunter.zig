...
pub const Hunter = struct {
    ...
    pub fn tick(self: *Hunter) !void {
        if (self.current_job) |job| {
            if (job.is_complete) {
                ...
                // Cleanup safely now that thread is done
                if (self.thread_handle) |t| t.detach();
    ...
    fn executeFetch(self: *Hunter, target: []const u8) !void {
        ...
        // [!] DEBOUNCE LOGIC
        // We check if a job exists using `|_|`.
        // We DO NOT touch the job data because the old thread might still be writing to it.
        // We just detach the thread and forget the pointer.
        if (self.current_job) |_| {
             if (self.thread_handle) |t| t.detach();
             self.current_job = null; 
        }
