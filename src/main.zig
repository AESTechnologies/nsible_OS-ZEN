...
//   module: "Kernel Root",
//   version: "v0.11.15-apex // Banysang",
//   description: "Primary initialization, rendering loop, and sovereign identity trap.",
//   changes: "Phase 1 Satori Resolution: Motor cortex bifurcation. Arrow controls and FL/FS reflexes now route contextually to Composer or Hunter. Macro erasure collision fixed.",
...
pub fn main() !void {
    ...
    fs.makeDir("assets/assist") catch |err|
    { if (err != error.PathAlreadyExists) {} };
        ...
        if (codex.transcieve(vinculum_fd)) |byte|
        {
            ...
            if (byte == 27) {
                ...
                if (esc_len < 8) {
                    ...
                    if (esc_len == 3 and esc_seq[1] == '[') {
                        if (byte == 'A' or byte == 'B' or byte == 'C' or byte == 'D') {
                            if (sys_composer.active) {
                                if (sys_composer.is_scan_active) {
                                    if (byte == 'A') { if (sys_composer.scan_line_y > 1) sys_composer.scan_line_y -= 1; }
                                    else if (byte == 'B') { sys_composer.scan_line_y += 1; }
                                    else if (byte == 'C') { sys_composer.focus_x += 8; sys_composer.focus_len = 1; }
                                    else if (byte == 'D') { if (sys_composer.focus_x > 56) sys_composer.focus_x -= 8; sys_composer.focus_len = 1; }
                                } else {
                                ...
                                if (sys_hunter.lens.is_scan_active) {
                                    ...
                                    else if (byte == 'C') { sys_hunter.lens.focus_x += 8; sys_hunter.lens.focus_len = 1; }
                                    else if (byte == 'D') { if (sys_hunter.lens.focus_x > 10) sys_hunter.lens.focus_x -= 8; sys_hunter.lens.focus_len = 1; }
                                } else {
                        ...
                        if (sys_composer.active) {
                            if (sys_composer.is_scan_active) {
                                if (byte == 'C') { sys_composer.focus_len += 1; }
                                else if (byte == 'D') { if (sys_composer.focus_len > 1) sys_composer.focus_len -= 1; }
                            }
                        } else if (!is_radio_modal and !is_calc_modal and !is_memo_modal and !is_trail_modal and !is_tabula_rasa and !is_bash_modal) {
                            if (sys_hunter.lens.is_scan_active) {
                                if (byte == 'C') { sys_hunter.lens.focus_len += 1; }
                                else if (byte == 'D') { if (sys_hunter.lens.focus_len > 1) sys_hunter.lens.focus_len -= 1; }
                            }
                        }
                        esc_len = 0;
                        continue;
			...
			if (std.mem.eql(u8, &seq_buf, ".!..-.")) {
                ...
                if (sys_composer.active) {
                    ...
                    sys_composer.is_scan_active = !sys_composer.is_scan_active;
                    if (sys_composer.is_scan_active) {
                        sys_composer.scan_line_y = 1;
                        sys_composer.focus_x = 56;
                        sys_composer.focus_len = 1;
                        sys_composer.setStatus("[ SATORI OPTICS : ACTIVE ]");
                    } else {
                        sys_composer.setStatus("[ SATORI OPTICS : OFFLINE ]");
                    }
                } else {
                ...
                // Phase 1 FIX: Erase the Satori strike macro from journal BEFORE appending payload
                if (journal_len >= 5) journal_len -= 5 else journal_len = 0;

                if (sys_composer.active) {
                    if (sys_composer.is_scan_active) {
                        // SATORI STRIKE: Placeholder spanning logic until Phase 2 connects
                    ...
                    if (sys_hunter.lens.is_scan_active) {
                        // SATORI STRIKE: Placeholder spanning logic until Phase 3 connects
                        var satori_buf: [128]u8 = undefined;
