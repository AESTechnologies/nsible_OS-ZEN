...
//   changes: "Phase 1 Satori CODEP: Bifurcated Motor Cortex routing, Shift+Arrow extending, fixed Macro Erasure collision, preserved osColors traps.",
...
fn drawHeader(is_high: bool) void {
    ...
    if (@"aud.state.io".is_active) {
        ...
        while (v_i < 8) : (v_i += 1) {
            ...
            var b_color: u32 = codex.get("K.S"); // Inactive dark grey
    ...
    while (true) {
        ...
        if (codex.transcieve(vinculum_fd)) |byte| {
            ...
            if (byte == 27) {
                ...
                if (esc_len < 8) {
                    ...
                    if (esc_len == 3 and esc_seq[1] == '[') {
                        ...
                        // [!] PHASE 1: Satori Span Extender (Shift+Arrow)
                        if (sys_composer.active) {
                            if (sys_composer.is_scan_active) {
			...
			if (std.mem.eql(u8, &seq_buf, ".!..-.")) {
                ...
                // [!] PHASE 1: Bifurcated FL Scan Trigger
                if (sys_composer.active) {
